...
export default function WinBanner({ type, visible, onClose }: WinBannerProps) {
        ...
        <div className="relative animate-bounce-5 text-center px-8 py-10 rounded-3xl bg-gradient-to-br from-purple-600 via-pink-500 to-yellow-400 shadow-2xl">
          ...
          {onClose && (
            <button
              ...
              className="mt-6 w-full rounded-xl bg-white/20 px-6 py-3 text-lg font-bold text-white hover:bg-white/30 focus:outline-none focus:ring-2 focus:ring-white"
            >
              Close
            </button>
          )}
  ...
  return (
    <div
      ...
      aria-live="polite"
      aria-atomic="true"
      className="fixed top-0 left-0 right-0 z-40 flex flex-col items-center justify-center py-6 bg-gradient-to-r from-blue-600 via-cyan-500 to-blue-600 animate-pulse shadow-lg"
    >
      ...
      {onClose && (
        <button
          ...
          className="mt-3 rounded-xl bg-white/20 px-6 py-2 text-base font-bold text-white hover:bg-white/30 focus:outline-none focus:ring-2 focus:ring-white"
        >
          Close
        </button>
      )}
